...
int main(){
	...
	//Thread A - In -> bufA	
	pthread_create(&threads[1], NULL, pipeline, createArgs(1,in,bufA,WORKLOAD1));
	
	
	
	//Thread B - bufA -> bufB
	pthread_create(&threads[2], NULL, pipeline, createArgs(2,bufA,bufB,WORKLOAD2));
	
	
	
	//Thread C - bufB -> out
	pthread_create(&threads[3], NULL, pipeline, createArgs(3,bufB,out,WORKLOAD3));
	 ...
	 */	
	double totaltime, latency, minLatency, maxLatency, avgLatency, throughput;
	totaltime = 0;
	latency = 0;
	...
	avgLatency = totaltime / N_DATA;
	throughput = 1000000 / avgLatency;
